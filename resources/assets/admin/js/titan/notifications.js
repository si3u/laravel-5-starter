function getHeaderNotifications()
{
    function getHeaderActivities()
    {
        ...
        function renderActivities(response, type)
        {
            ...
            if (items.length > 0) {
                ...
                $('#js-' + type + '-badge').hide();
                $('#js-' + type + '-list').html('<li><a><p style="margin-left: 0px; text-align: center">Il n\'existe aucun <b>' + type + '</b></p></a></li>');
                return false;
            }
    ...
    function renderNotificationsTable(response)
    {
        ...
        if (items.length > 0) {
            ...
            $('#tbl-notifications').find('tbody').html('<tr><td colspan="3" class="text-center">Il n\'y a actuellement aucune notification</td></tr>');
            return false;
        }
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            html += '<td><a data-user="' + item['user_id'] + '" data-id="' + item['id'] + '" class="btn btn-xs btn-primary btn-notification-read">Marqué comme Lu</a></td></tr>';
