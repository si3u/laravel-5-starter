...
class LoginController extends TitanController
{
    ...
    public function login(Request $request)
    {
        ...
        if (!User::where($this->username(), input($this->username()))
            ...
            return $this->sendFailedLoginResponse($request,
                'Activé votre compte pour pouvoir vous connecter.');
        }
    ...
    private function sendLoginResponse(Request $request)
    {
        ...
        if ($user->getAttribute('logged_in_at')) {
            notify()->info('Info',
                'Dernière connexion : ' . $user->getAttribute('logged_in_at')
                    ->diffForHumans(), 'clock-o rotateIn animated', 8000);
        }
        else {
            notify()->info('Bienvenue',
                'Bonjour ' . $user->getAttribute('fullname') . '. Bienvenue sur ' . config('app.name'),
        ...
        log_activity('Login', user()->fullname . ' s\'est Connecté.');
